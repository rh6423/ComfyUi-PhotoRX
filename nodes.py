"""
ComfyUI nodes for PhotoRX film grain rendering.

This module provides three node types:
  - PhotoRXFilmGrainBasic: Simple interface with preset selection
  - PhotoRXFilmGrainAdvanced: Full control over all parameters
  - PhotoRXProfileInfo: Display information about a film profile

All nodes support batch processing and preserve GPU tensors when possible.
"""
    ...
    if alpha_channel is not None:
        ...
        if zoom != 1.0:
            ...
            alpha_pil = PILImage.fromarray(
                np.clip(alpha_channel * 255, 0, 255).astype(np.uint8), mode='L'
            )
            new_size = (int(result_rgb.shape[1] * zoom), int(result_rgb.shape[0] * zoom))
            alpha_pil = alpha_pil.resize(new_size, PILImage.BILINEAR)
            alpha_channel = np.array(alpha_pil).astype(np.float32) / 255.0
...
class PhotoRXFilmGrainBasic:
    """
    Basic film grain node with preset selection.
    ...
    FUNCTION_NAME = "PhotoRX Film Grain Basic"
    ...
    def IS_CHANGED(self, **kwargs) -> float:
        """Return seed for caching purposes."""
        return float(kwargs.get("seed", 42))
    ...
    def execute(self, image: torch.Tensor, profile: str, strength: float,
        ...
        """
        Apply film grain to input image using basic settings.
...
class PhotoRXFilmGrainAdvanced:
    ...
    FUNCTION_NAME = "PhotoRX Film Grain Advanced"
    CATEGORY = "PhotoRX/Effects"
...
class PhotoRXProfileInfo:
    """
    Display information about a film profile.
    ...
    Outputs:
        info: Dictionary with profile information
"""

    FUNCTION_NAME = "PhotoRX Profile Info"
